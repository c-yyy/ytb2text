...
```bash
npm run yt                      # 真实 YouTube 观看页，18 项断言
...
都会被抓出来）、悬浮球有没有收起、点击能否开合、**节点被清空后会不会自动补挂**、
**把那一行折叠掉之后入口会不会改挂到可见位置**，以及 popup 的「检测本页」链路是否通。

入口没出现时，点扩展图标 →「检测本页」，它会逐条告诉你卡在哪一步
（站点规则没命中 / 操作栏没找到 / 插了但不可见 / 用的是兜底样式 / 悬浮球在不在）。
...
3. 打开有视频的网页：YouTube 观看页会在「分享 / 保存」那一行多出**「转文字」按钮**；
   其它页面用右下角的靛蓝色悬浮按钮
...
> popup 右上角显示的版本号（`v0.2.2`）可以用来确认重载是否真的生效。
...
页面：www.youtube.com
✓ 命中站点规则：youtube
✓ 找到可挂载的操作栏（top-level-buttons-computed）
✓ 入口已插入 DOM
· 悬浮按钮已收起（入口正常）
操作栏探测：flexible=折叠（共 3 个）｜topLevel=可见
→ 一切正常。
```

- **`✗ 这个站点没有配置原生入口`** → 目前只给 YouTube 观看页配了操作栏入口，
  其它站点用右下角悬浮按钮（点「在当前页面打开面板」也行）。
- **`✗ 没找到可挂载的操作栏`** → 页面还没渲染完，或那一组被折叠了；
  悬浮球可兜底，稍等会自动补挂。
- **`✗ 插进去了但不可见`** → 就是这个 bug 本身：容器被折叠了。正常版本会自动改挂。
- **`✗ 也看不到悬浮按钮`** → 在扩展页点「重新加载」后刷新本页。
...
| **页面内视频** | 页面里的 `<video>` / `<audio>` | 见下 |

「页面内视频」会**按媒体地址自动分两种打法**：

- **http(s) 直链** → 直接下载原始文件再解码，几秒就能出结果，按钮显示 **「转录」**；
  下载失败（跨域、要登录、分片流）会**自动改走**下面这条，不会给你一个死胡同。
- **`blob:` / MSE 流媒体**（YouTube、B站、腾讯视频都这样，边下边播）→ 拿不到原始文件，
  但声音是有的：用 `HTMLMediaElement.captureStream()` 把播放器的音轨接出来录。
  按钮显示 **「边播边录」**，代价是只能按播放速度实时录（MSE 的物理限制）。
  好处是不需要额外权限，也不会把别的标签页声音录进去。
...
- **实时录制**：「录制标签页」和「边播边录」都是按播放速度走的 —— 10 分钟视频要播 10 分钟。
  想快只能用「本地文件」或 http(s) 直链的「页面内视频」。
- **加密流媒体拿不到**（奈飞、Disney+ 这类 DRM 内容）：元素上没有可捕获的音轨，
  只能走系统级的「录制标签页」。
